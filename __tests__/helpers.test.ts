...
describe('helpers.ts', () => {
  ...
  describe('validatePath', () => {
    ...
    it('returns an absolute path for any valid input', () => {
      const result = validatePath('subdir/nested/file.json')
      expect(path.isAbsolute(result)).toBe(true)
    })

    it('error message includes original input path', () => {
      const dangerousPath = '../secret.txt'
      expect(() => validatePath(dangerousPath)).toThrow(dangerousPath)
    })

    it('throws for root directory (absolute path outside cwd)', () => {
      expect(() => validatePath('/')).toThrow('Path traversal detected')
    })
  })
...
X-Bearer: only-bearer-no-token`
    ...
    it('masks X-Bearer-Token because it contains "token" (still in sensitivePatterns)', () => {
      const yamlInput = 'X-Bearer-Token: my-jwt-value'

      const result = parseCustomHeaders(yamlInput)

      expect(result).toEqual({'X-Bearer-Token': 'my-jwt-value'})
      // "token" is in sensitivePatterns, so X-Bearer-Token must be masked
      expect(core.debug).toHaveBeenCalledWith('Custom header added: X-Bearer-Token: ***MASKED***')
    })

    it('does not mask X-Service-Credential because "credential" was removed from sensitivePatterns', () => {
      const yamlInput = 'X-Service-Credential: some-value'

      const result = parseCustomHeaders(yamlInput)

      expect(result).toEqual({'X-Service-Credential': 'some-value'})
      // "credential" is no longer in sensitivePatterns
      expect(core.debug).toHaveBeenCalledWith('Custom header added: X-Service-Credential: some-value')
      expect(core.debug).not.toHaveBeenCalledWith(expect.stringContaining('***MASKED***'))
    })

    it('does not mask X-Bearer-Only because "bearer" was removed from sensitivePatterns (no other pattern matches)', () => {
      const yamlInput = 'X-Bearer-Only: standalone-bearer'

      const result = parseCustomHeaders(yamlInput)

      expect(result).toEqual({'X-Bearer-Only': 'standalone-bearer'})
      // "bearer" alone is not in sensitivePatterns; "token" is not in header name
      expect(core.debug).toHaveBeenCalledWith('Custom header added: X-Bearer-Only: standalone-bearer')
      expect(core.debug).not.toHaveBeenCalledWith(expect.stringContaining('***MASKED***'))
    })

    it('still masks all five remaining sensitive patterns', () => {
