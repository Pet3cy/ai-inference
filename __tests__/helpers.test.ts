...
describe('helpers.ts', () => {
  ...
  describe('validatePath', () => {
    ...
    it('throws for absolute path to parent directory', () => {
      const parentDir = path.resolve(process.cwd(), '..')
      expect(() => validatePath(parentDir)).toThrow('Path traversal detected')
    })

    it('returns a string (resolved absolute path) for valid nested path', () => {
      const result = validatePath('a/b/c/d/file.txt')
      expect(typeof result).toBe('string')
      expect(path.isAbsolute(result)).toBe(true)
      expect(result).toBe(path.resolve(process.cwd(), 'a/b/c/d/file.txt'))
    })
  })
  ...
  describe('loadContentFromFileOrInput', () => {
    ...
    it('error message includes original (not safe) path when file is missing', () => {
...
X-Bearer: only-bearer-no-token`
    ...
    it('masks X-Bearer-Token because "token" is in sensitivePatterns', () => {
      // X-Bearer is NOT masked (bearer removed from patterns)
      // X-Bearer-Token IS masked because it contains "token" (still in patterns)
      const yamlInput = `X-Bearer: bearer-only
X-Bearer-Token: actual-token-value`

      const result = parseCustomHeaders(yamlInput)

      expect(result).toEqual({
        'X-Bearer': 'bearer-only',
        'X-Bearer-Token': 'actual-token-value',
      })

      expect(core.debug).toHaveBeenCalledWith('Custom header added: X-Bearer: bearer-only')
      expect(core.debug).toHaveBeenCalledWith('Custom header added: X-Bearer-Token: ***MASKED***')
    })

    it('does not mask headers whose names only match removed patterns (credential, bearer)', () => {
