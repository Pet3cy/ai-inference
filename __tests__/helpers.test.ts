...
describe('helpers.ts', () => {
  ...
  describe('validatePath', () => {
    ...
    it('resolves empty string to cwd without throwing', () => {
      const result = validatePath('')
      expect(result).toBe(path.resolve(process.cwd(), ''))
    })

    it('allows absolute path that is exactly equal to cwd', () => {
    ...
    it('error message includes the offending path', () => {
      const offendingPath = '../sneaky.txt'
      expect(() => validatePath(offendingPath)).toThrow(offendingPath)
    })
  })
...
X-Bearer: only-bearer-no-token`
    ...
    it('still masks X-Bearer-Token because it contains "token" (still in sensitivePatterns)', () => {
      const yamlInput = `X-Bearer-Token: abcdef123
X-Bearer: only-bearer-substring`

      const result = parseCustomHeaders(yamlInput)

      expect(result).toEqual({
        'X-Bearer-Token': 'abcdef123',
        'X-Bearer': 'only-bearer-substring',
      })

      // X-Bearer-Token contains "token" which is still sensitive → must be masked
      expect(core.debug).toHaveBeenCalledWith('Custom header added: X-Bearer-Token: ***MASKED***')
      // X-Bearer only contains "bearer" which was removed from sensitivePatterns → NOT masked
      expect(core.debug).toHaveBeenCalledWith('Custom header added: X-Bearer: only-bearer-substring')
    })

    it('still masks headers whose name matches remaining sensitive patterns (key, token, secret, password, authorization)', () => {
...
Authorization: Basic dXNlcjpwYXNz`
    ...
    it('does not mask a header whose name contains only "credential" or "bearer" substrings (removed from sensitivePatterns)', () => {
      ...
      // credential and bearer are no longer in sensitivePatterns
