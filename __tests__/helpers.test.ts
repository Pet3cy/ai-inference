...
describe('helpers.ts', () => {
  ...
  describe('validatePath', () => {
    ...
    it('allows a deeply nested path that ultimately resolves within cwd', () => {
      // a/b/c/../../../subdir resolves to cwd/subdir — still inside cwd
      const result = validatePath('a/b/c/../../../subdir/file.txt')
      expect(result).toBe(path.resolve(process.cwd(), 'subdir/file.txt'))
    })

    it('throws for a path that uses .. after crossing above cwd', () => {
      // starts with ../ so always above cwd
      expect(() => validatePath('../sibling-dir/file.txt')).toThrow('Path traversal detected')
    })
  })
...
X-Bearer: only-bearer-no-token`
    ...
    it('masks X-Bearer-Token because "token" is still in sensitivePatterns', () => {
      // "bearer" was removed from sensitivePatterns but "token" was not.
      // A header named X-Bearer-Token contains "token" and must still be masked.
      const result = parseCustomHeaders('X-Bearer-Token: abc123')
    ...
    it('does not mask X-Service-Credential because "credential" was removed from sensitivePatterns', () => {
      const result = parseCustomHeaders('X-Service-Credential: my-cred-value')

      expect(result).toEqual({'X-Service-Credential': 'my-cred-value'})
      expect(core.debug).toHaveBeenCalledWith('Custom header added: X-Service-Credential: my-cred-value')
