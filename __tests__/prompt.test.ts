 ...
 var2: |
  ...
  describe('parseFileTemplateVariables', () => {
    ...
    it('reads multiple file variables and returns all contents', () => {
      ...
      expect(data.schema).toContain('messages:')
      expect(data.schema).toContain('responseFormat:')
      expect(data.simple).toContain('role: system')
    })

    it('throws with correct error message when YAML root is not an object', () => {
      expect(() => parseFileTemplateVariables('"just a string"')).toThrow(
        'Failed to parse file template variables: File template variables must be a YAML object',
      )
    })

    it('throws with correct error message when YAML is a plain scalar (number)', () => {
      expect(() => parseFileTemplateVariables('42')).toThrow('Failed to parse file template variables')
    })
    ...
    it('error for missing file includes the variable name', () => {
      expect(() => parseFileTemplateVariables("myvar: ./does-not-exist-at-all.txt")).toThrow("'myvar'")
    })
  })
})
