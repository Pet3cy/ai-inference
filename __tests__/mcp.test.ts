...
describe('mcp.ts', () => {
  ...
  describe('executeToolCalls', () => {
    ...
    it('executes a single tool call correctly (boundary case for sequential loop)', async () => {
      const toolCalls = [{id: 'solo-call', type: 'function', function: {name: 'only-tool', arguments: '{}'}}]
      ...
      expect(results[0].name).toBe('only-tool')
      expect(results[0].content).toContain('Solo result')
      expect(mockCallTool).toHaveBeenCalledTimes(1)
    })

    it('returns error results for all tools when all fail (sequential robustness)', async () => {
      const toolCalls = [
        {id: 'fail-1', type: 'function', function: {name: 'bad-tool-1', arguments: '{}'}},
        {id: 'fail-2', type: 'function', function: {name: 'bad-tool-2', arguments: '{}'}},
      ]

      mockCallTool
        .mockRejectedValueOnce(new Error('Tool 1 error'))
        .mockRejectedValueOnce(new Error('Tool 2 error'))

      const results = await executeToolCalls(mockClient, toolCalls)

      // Both tools failed but execution continued for both
      expect(results).toHaveLength(2)
      expect(results[0].tool_call_id).toBe('fail-1')
      expect(results[0].content).toContain('Error:')
      expect(results[1].tool_call_id).toBe('fail-2')
